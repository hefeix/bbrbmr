...
::Matrix LeftDiv( const MatrixView& ma, const MatrixView& mb ) // ma\mb =approx= inv(ma)*mb
{
    TNT::Array2D<double> a( ma.nRows(), ma.nCols() );
    for( int r=0; r<ma.nRows(); r++ )
        for( int c=0; c<ma.nCols(); c++ )
            a[r][c] = ma.val(r,c);

    JAMA::QR<double> qr( a );
    if( !qr.isFullRank() )
        throw MatrixExc("Degenerate matrix, left division failed");  //--->>--

    TNT::Array2D<double> b( mb.nRows(), mb.nCols() );
    for( int r=0; r<mb.nRows(); r++ )
        for( int c=0; c<mb.nCols(); c++ )
            b[r][c] = mb.val(r,c);

    TNT::Array2D<double> aLDb = qr.solve( b );
    if( 0==aLDb.dim1() || 0==aLDb.dim2() )
        throw MatrixExc("Non-conformant matrix, left division failed");  //--->>--

    ::Matrix res( aLDb.dim1(), aLDb.dim2() );
    for( int rr=0; rr<aLDb.dim1(); rr++ )
        res[ res.row(rr) ] = valarray<double>( aLDb[rr], aLDb.dim2() );

    return res;
}

::Vector VectorLeftDiv( const MatrixView& ma, const ::Vector& mb ) // ma\mb =approx= inv(ma)*mb
{
    if( ma.nRows() != mb.size() )
        throw DimensionConflict(__FILE__,__LINE__);
    ...
    JAMA::LU<double> lu( a );
    if( !lu.isNonsingular() )
        throw MatrixExc("Degenerate matrix, vector left division failed");  //--->>--

    TNT::Array1D<double> b( mb.size() );
    for( unsigned r=0; r<mb.size(); r++ )
        b[r] = mb[r];

    TNT::Array1D<double> aLDb =lu.solve( b );
    if( 0==aLDb.dim1() )
        throw MatrixExc("Non-conformant matrix, left division failed");  //--->>--
...
//<<-------------------------- end from GNU GLS package ---------------------
double densnorm(double x) {
