 ...
 * 2.01   Jan 31, 06    fixed bug: Gaussian penalty should be 1/(2*var), not 1/var
 * 2.03   Mar 16, 07    fixed bug with "infinite penalty", detected by Shenzhi Li
 */
    ...
    for( itr=0; !converge; itr++ ) //---iterations loop--
    {
        ...
        //for( unsigned j=0; j<d; j++ ) //--design vars--
        {
            ...
            //for( unsigned k=0; k<c; k++ ) //--classes--
            {
                ...
                if( normal==bayesParam.PriorType() )
                {
                    double dw_numerator = (w(j,k)==priorMean(j,k)) ? 0.0  //in case of infinite penalty //ver 2.03
                        : - 2 * ( w(j,k) - priorMean(j,k) ) * penalty(j,k);
                    ...
                    Stats& stats )
{
    ...
    if( ! hyperParamPlan.hasGrid() ) {    //==paramPlan.size() ) {
        ...
        drs.rewind();
        m_beta = ParamMatrixSquare( drs.dim(), drs.c(),  0.0);
...
//#define HESSIAN
    ...
    m_bTrained = true;
    modelFile.WriteParams( m_modelType, designParameter, m_pDesign, m_featSelect, m_beta );
